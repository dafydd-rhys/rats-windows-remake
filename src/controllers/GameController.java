...
public class GameController implements Initializable {
    ...
    private static void move(final Rat rat) {
        ...
        Movement.rat = rat;
        Movement.current =
                level.getTiles()[rat.getCurrentPosX()][rat.getCurrentPosY()];
        ...
        if (currentTick % 2 == 0) {
            for (int i = 0; i < rats.size(); i++) {
                if (rats.get(i).isAdult()) {
                    ...
                    move(rat);
                    rat.findPartner(
                            level.getTiles()[rat.getCurrentPosY()][rat.getCurrentPosX()]);
                    rat.giveBirth();
                }
            }
        }
        ...
        Platform.runLater(() -> {
            ...
            for (Rat rat : rats) {
                if (level.getTiles()[rat.getCurrentPosY()][rat.getCurrentPosX()].isCovering()) {
                    gc.drawImage(rat.getRotatedImage(),
                            rat.getCurrentPosX() * 50,
                            rat.getCurrentPosY() * 50);
                }
            }
            ...
            for (Item item : items) {
                if (level.getTiles()[item.getCurrentPosY()][item.getCurrentPosX()].isCovering()) {
                    if (item.getType() == Item.TYPE.DEATH_RAT) {
                        ...
                        gc.drawImage(deathRat.getRotatedImage(),
                                item.getCurrentPosX() * 50 + 10,
                        ...
                        for (Tile tile : gas.getTiles()) {
                            if (tile.isCovering()) {
                                gc.drawImage(smoke, tile.getX() * 50,
                                        tile.getY() * 50);
                            }
                        }
                    } else {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            levelReader =
                    new LevelFileReader(Level.currentLevel, Level.isSave());
            if (Level.isSave()) {
        ...
        Canvas canvas = new Canvas(levelReader.getSizeX() * 50,
                levelReader.getSizeY() * 50);
        ...
        if (!Level.isSave()) {
            ...
            LevelFileGenerator generator =
                    new LevelFileGenerator(levelReader.getTimeToGenerate(), gc,
                            levelReader.getSizeX(),
                            levelReader.getSizeY(), levelReader.getLevel(),
                            levelReader.getSpawns(),
                            levelReader.getExpectedTime(),
                            levelReader.getMaxRats());
            level = generator.getLevel();
        } else {
            assert levelLoader != null;
            LevelLoadGenerator loadGenerator =
                    new LevelLoadGenerator(levelReader.getTimeToGenerate(), gc,
                            levelReader.getSizeX(),
                            levelReader.getSizeY(), levelReader.getLevel(),
                            levelLoader.getRatSpawns(),
                            levelLoader.getItemSpawns(),
                            levelReader.getExpectedTime(),
                            levelReader.getMaxRats());
        ...
        lblExpected.setText(
                "Expected: " + level.getExpectedTime() + " seconds");
        ...
        TimerTask task = new TimerTask() {
            ...
            public void run() {
                if (Level.getPaused()) {
                    currentTick += 1;
                    if (currentTick % 2 == 0) {
                        lblTime.setText("Current: " + (int) currentTick / 2 +
                                " seconds");
                    }
                    ...
                    if (level.getRats().size() >= level.getMaxRats()) {
                        ...
                        if ((int) currentTick / 2 < level.getExpectedTime()) {
                            int timeLeft = (int) (level.getExpectedTime() -
                                    (currentTick / 2));
                    ...
                    if (Level.getGameOver()) {
                        ...
                        if (Level.getGameWon()) {
                            try {
                                Score score = new Score(Level.getCurrentLevel(),
