...
public class PlayerFunction {
    ...
    public void addPlayerItems(int ItemId, int a) {
        if (player.getMaxNumberOfItems()[ItemId] > player.getPlayerItems()[ItemId]) {//check is player have max item
            if (player.getPlayerItems().length == 0) {//if the array is empty, input the default value
                double[] tempList = {0, 0, 0, 0, 0, 0, 0, 0};
                player.setPlayerItems(tempList);
            } else {//add item to playerItems.s
